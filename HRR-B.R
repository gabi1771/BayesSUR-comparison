...
hyperpar <- list(a_omega=1.0,b_omega=3.0)
#Run the model
set.seed(28173)
tic("Timeofmodelfitting")
fit<-BayesSUR(Y=Y,X=X,outFilePath="HRRB",nIter=200000,
              nChains=3, burnin=100000,
              covariancePrior=covariancePrior,
              gammaPrior=gammaPrior,hyperpar=hyperpar,
              output_CPO = TRUE)
toc()
...
#MCMC diagnostics
pdf("HRRB.MCMC_diagnostic.pdf")
plot(fit, estimator = "logP", type = "diagnostics")
dev.off()


#Estimators
pdf("HRRB.pdf",width=12,height=4)
...
existing_data <- read_excel("Results_HRRB.xlsx", sheet = "Sheet1")
...
write_xlsx(combined_data, path = "Results_HRRB.xlsx")

# Message de confirmation
cat("Les données ont été ajoutées au fichier Results_HRRB.xlsx")
