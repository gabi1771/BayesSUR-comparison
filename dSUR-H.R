...
hyperpar <- list(a_o=1,b_o=1,a_pi=1,b_pi=1)  

#Run the model
set.seed(28173)
tic("Timeofmodelfitting")
fit<-BayesSUR(Y=Y,X=X,outFilePath="dSURH",nIter=200000,
...
#MCMC diagnostics
pdf("dSURH.MCMC_diagnostic.pdf")
plot(fit, estimator = "logP", type = "diagnostics")
dev.off()


#Estimators
pdf("dSURH.pdf",width=12,height=4)
...
existing_data <- read_excel("Results.xlsx", sheet = "Sheet1")
...
write_xlsx(combined_data, path = "Results.xlsx")

# Message de confirmation
cat("Les données ont été ajoutées au fichier Results.xlsx")
