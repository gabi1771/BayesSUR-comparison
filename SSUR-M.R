...
hyperpar <- list(a_eta=3.5,b_eta=11,mrf_d=-2.5,mrf_e=0.6)  

#Run the model
set.seed(28173)
tic("Timeofmodelfitting")
fit<-BayesSUR(Y=Y,X=X,outFilePath="SSURM",nIter=200000,
...
#MCMC diagnostics
pdf("SSURM.MCMC_diagnostic.pdf")
plot(fit, estimator = "logP", type = "diagnostics")
dev.off()


#Estimators
pdf("SSURM.pdf",width=12,height=4)
