...
hyperpar <- list(a_o=3.0,b_o=6.0,a_pi=2,b_pi=3,a_eta=1.3,b_eta=2.5)#Run the model
set.seed(28173)
tic("Timeofmodelfitting")
fit<-BayesSUR(Y=Y,X=X,outFilePath="SSURH",nIter=200000,
...
#MCMC diagnostics
pdf("SSURH.MCMC_diagnostic.pdf")
plot(fit, estimator = "logP", type = "diagnostics")
dev.off()


#Estimators
pdf("SSURH.pdf",width=12,height=4)
